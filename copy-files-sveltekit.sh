...
IGNORE_DIRS="node_modules|build|\.git|\.svelte-kit"
export IGNORE_FILES IGNORE_DIRS

# Copy specific files from the root directory
find . -maxdepth 1 -type f \( -name ".gitignore" -o -name "tailwind.config.js" -o -name "tsconfig.json" -o -name ".prettierrc" -o -name "svelte.config.js" -o -name "package.json" -o -name "README.md" \) -not -regex ".*\(${IGNORE_FILES}\)" -exec bash -c 'copy_file "$0"' {} \;

# Copy specific file types from the src directory and its subdirectories
find src -type f \( -name "*.js" -o -name "*.ts" -o -name "*.svelte" -o -name "*.json" -o -name "*.md" \) -not -path "*/node_modules/*" -not -path "*/build/*" -not -path "*/.git/*" -not -path "*/.svelte-kit/*" -not -regex ".*\(${IGNORE_FILES}\)" -exec bash -c 'copy_file "$0"' {} \;
...
tree -a -I "${IGNORE_DIRS}|${IGNORE_FILES}|copy" --dirsfirst >copy/SVELTEKIT_PROJECT_STRUCTURE.txt
echo "Project structure has been saved to copy/SVELTEKIT_PROJECT_STRUCTURE.txt"
